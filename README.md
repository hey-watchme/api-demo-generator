...
サンプルデバイスとは、**device_type = "demo"** のすべてのデバイスを指します。

このAPIで生成されるデモデータは、iOSアプリにおいて以下の目的で使用されます：

1. **初回ユーザー体験の向上**
   - 新規登録ユーザーに、アプリの機能をすぐに体験してもらう
   - デフォルトサンプルデバイス（`child_5yo`）が自動的に追加される

2. **事例カタログとしての活用**
   - 3歳児、20歳、70代女性など、様々な年齢層のデータを閲覧可能
   - QRコードスキャンで追加サンプルデバイスを追加できる
...
- **Persona**: `child_5yo`（5歳男児）
- **特徴**:
  - すべてのユーザーに最初から表示される
  - ユーザー登録時に自動的に `user_devices` テーブルに追加される
  - 連携解除可能（後で再追加も可能）

#### 2. 追加サンプルデバイス
- **Device ID**: `00000000-0000-0000-0001-000000000002`, `00000000-0000-0000-0001-000000000003`, etc.
- **Persona**: `adult_30s`, `elderly_70s`, etc.
- **特徴**:
  - QRコードスキャンで追加する
  - `user_devices` テーブルに登録されているユーザーのみ閲覧可能
  - 連携解除可能

### iOSアプリでの扱い（実装済み）

#### データベース管理
- **統一管理**: サンプルデバイスも通常デバイスと同じく `user_devices` テーブルで管理
- **特別扱いなし**: 内部的に特別なロジックは不要（`device_type` で判定）

#### UI表示
- **セクション分け**:
  1. Observer デバイス（`device_type = "observer"`）
  2. iPhone デバイス（`device_type = "ios"` or `"android"`）
  3. **サンプルデバイス**（`device_type = "demo"`）
- 各セクションはデバイスが存在する場合のみ表示

#### 操作制限
- ✅ **可能な操作**:
  - 選択（データ閲覧）
  - 連携解除（`user_devices` から削除）
  - 再追加（QRコードスキャン）
- ❌ **不可能な操作**:
  - デバイス削除（`devices` テーブルから削除）
  - 観測対象の編集・追加
  - デバイス情報の編集

#### ユーザー登録時の自動セットアップ
```swift
// UserAccountManager.swift - initializeAuthenticatedUser()
// 1. ユーザーデバイスを登録
await deviceManager.registerDevice(userId: userId)

// registerDevice() の内部処理:
// - Step 1: devicesテーブルにiOSデバイスを登録
// - Step 2: user_devicesテーブルにowner関係を登録
// - Step 3: デフォルトサンプルデバイスをviewer権限で自動追加
```

### データ生成の詳細

詳細は「**生成されるデータ構造**」セクションを参照。
...
|----|------|-----------|------|---------|
| `child_5yo` | 5歳男児 | `a1b2c3d4-e5f6-4a5b-8c9d-0e1f2a3b4c5d` | 幼稚園年長、マインクラフト好き（**デフォルトサンプル**） | ✅ 完了 |
